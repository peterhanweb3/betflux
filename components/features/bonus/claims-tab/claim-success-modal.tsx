...
export const ClaimSuccessModal = (props: ClaimSuccessModalProps) => {
	const { user } = useDynamicAuth();
	const { isOpen, onClose, claimedBonusAmount } = props;
	const t = useTranslations("bonus.claimSuccessModal");
	...
	const shareText = t("shareText", {
		amount: claimedBonusAmount.toFixed(2),
		url: shareUrl,
	});
	...
	const handleCopyReferral = async () => {
		if (user?.referralId) {
			try {
				...
				toast.error(t("referralLinkCopyFailed"));
			}
		}
	};
						...
						<div className="z-10 space-y-4 sm:space-y-2">
							...
							<div>
								<h3 className="text-6xl sm:text-7xl md:text-8xl font-black text-emerald-400 leading-none tracking-tight">
									+{Number(claimedBonusAmount).toFixed(2)}
								</h3>
							</div>
